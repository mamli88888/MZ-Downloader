...
class YouTubeFormatSizeEstimationTests(unittest.TestCase):
    def setUp(self) -> None:
        self.formats = (
            YouTubeFormatSize(height=360, abr_kbps=128, is_audio_only=False, is_muxed=True, size=10_000_000),
            YouTubeFormatSize(height=720, abr_kbps=None, is_audio_only=False, is_muxed=False, size=25_000_000),
            YouTubeFormatSize(height=1080, abr_kbps=None, is_audio_only=False, is_muxed=False, size=45_000_000),
            YouTubeFormatSize(height=None, abr_kbps=128, is_audio_only=True, is_muxed=False, size=3_000_000),
            YouTubeFormatSize(height=None, abr_kbps=48, is_audio_only=True, is_muxed=False, size=1_500_000),
        )

    def test_muxed_format_is_used_directly_without_audio_bonus(self) -> None:
        size = estimate_youtube_size(self.formats, is_audio=False, target_height=360)
        self.assertEqual(size, 10_000_000)

    def test_video_only_format_gets_best_audio_track_added(self) -> None:
        size = estimate_youtube_size(self.formats, is_audio=False, target_height=1080)
        self.assertEqual(size, 45_000_000 + 3_000_000)

    def test_closest_height_is_chosen_when_exact_is_missing(self) -> None:
        size = estimate_youtube_size(self.formats, is_audio=False, target_height=800)
        self.assertEqual(size, 25_000_000 + 3_000_000)

    def test_audio_matches_closest_bitrate(self) -> None:
        size = estimate_youtube_size(self.formats, is_audio=True, target_bitrate_kbps=64)
        self.assertEqual(size, 1_500_000)

    def test_missing_data_returns_none_instead_of_guessing(self) -> None:
        self.assertIsNone(estimate_youtube_size((), is_audio=False, target_height=720))
        self.assertIsNone(estimate_youtube_size((), is_audio=True, target_bitrate_kbps=128))
        audio_only_formats = tuple(item for item in self.formats if item.is_audio_only)
        self.assertIsNone(estimate_youtube_size(audio_only_formats, is_audio=False, target_height=720))
