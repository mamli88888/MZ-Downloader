    ...
    try:
        ...
        for bot_username in bots_to_try:
            try:
                ...
                # Step 1: Identify with @Musicfindmhdbot
                if bot_username.lower() == "musicfindmhdbot":
                    if result.status == "needs_selection":
                        option = next((opt for opt in result.options if "شناسایی موسیقی" in opt.label), None)
                        if option:
                            await progress.update(30, "🔍 دارم موزیک رو شناسایی می‌کنم…", force=True)
                            # After clicking, the bot edits the caption to include "Song — Artist"
                            result = await GATEWAY.select(
                                client=lease.worker.client,
                                worker_name=lease.worker.name,
                                bot_username=bot_username,
                                ...
                                attempt_directory=attempt_directory,
                                progress_callback=progress.download,
                                expected_kind_override=MediaKind.NONE, # We expect a text edit
                            )
                            
                            # Step 2: Extract "Song — Artist" from the edited caption
                            song_query = None
                            if result.status == "text" and " — " in result.text:
                                # Look for the specific format "Song — Artist" in the text
                                lines = result.text.split("\n")
                                for line in lines:
                                    if " — " in line:
                                        song_query = line.strip()
                                        break
                            
                            if song_query:
                                await progress.update(50, f"🔎 دارم آهنگ «{song_query}» رو از ربات دوم می‌گیرم…", force=True)
                                # Step 3: Send the song query to the second bot (@whatisthismusicbot)
                                second_bot = "whatisthismusicbot"
                                result = await GATEWAY.request(
                                    client=lease.worker.client,
                                    worker_name=lease.worker.name,
                                ...
                                # Step 4: Click the first button in the second bot's response
                                if result.status == "needs_selection":
                                    await progress.update(70, "📥 دارم فایل آهنگ رو دانلود می‌کنم…", force=True)
                                    result = await GATEWAY.select(
                                        client=lease.worker.client,
                                        worker_name=lease.worker.name,
                                        bot_username=second_bot,
                                        request_message_id=result.request_message_id,
                                        menu_message_id=result.menu_message_id,
                                        option=result.options[0], # First button
                                        attempt_directory=attempt_directory,
                                        progress_callback=progress.download,
                                    )
                                    
                                    # Handle "Not found" or error from the second bot
                                    if result.status != "ready":
                                        # If search failed, try sending the direct URL to the second bot
                                        await progress.update(80, "⚠️ آهنگ پیدا نشد، دارم لینک مستقیم رو تست می‌کنم…", force=True)
                                        result = await GATEWAY.request(
                                            client=lease.worker.client,
                                            worker_name=lease.worker.name,
                                            bot_username=second_bot,
