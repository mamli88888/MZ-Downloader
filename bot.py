...
REQUIRED_CHANNELS = (
    ("@MZBOTS_Monitor", "https://t.me/MZBOTS_Monitor", "MZBOTS Monitor"),
    ("@ImagePromptLab", "https://t.me/ImagePromptLab", "ImagePromptLab"),
)
...
async def _process_url(
    ...
    if platform is None:
        ...
        return
    is_spotify_album = platform == Platform.SPOTIFY and spotify_resource_type(url) == "album"
    normal_providers = () if is_spotify_album else providers_for_platform(platform, SETTINGS)
    providers = tuple(dict.fromkeys((*normal_providers, *all_providers(SETTINGS))))
    if not providers:
        STATS.failed += 1
        return
    info = platform_info(platform)
    if not is_spotify_album and ACCOUNT_POOL.queue_length >= SETTINGS.max_queue_size:
        STATS.failed += 1
        await send_status(
            context,
            chat_id,
        ...
        return
    if not is_spotify_album and ACCOUNT_POOL.total == 0:
        STATS.failed += 1
        await send_status(
            context,
            chat_id,
    ...
    try:
        if is_spotify_album:
            ...
            async def album_progress(percent: int, detail: str) -> None:
                safe_detail = detail.replace("Spotisaver", "منبع آهنگ")
                await progress.update(
                    5 + int(percent * 0.65),
                    "🎵 دارم آلبوم Spotify رو آماده می‌کنم…",
                    safe_detail,
                    force=percent in {2, 90, 100},
                )

            try:
                album = await SpotisaverAlbumDownloader(
                    proxy_url=_caption_proxy_url(),
                ).download_album(url, attempt_directory, progress=album_progress)
                album_media = DownloadedMedia(
                    path=album.path,
                    kind=MediaKind.DOCUMENT,
                    source_message_id=0,
                    mime_type="application/zip",
                    size=album.path.stat().st_size,
                )
                await send_result_to_user(
                    update,
                    context,
                    status_message,
                    GatewayResult(status="ready", bot_username="spotisaver.net", media=(album_media,)),
                    reply_to=reply_to,
                    request_id=request_id,
                    progress=progress,
                )
                if album.failed_tracks:
                    await context.bot.send_message(
                        chat_id=chat_id,
                        text=status_card(
                            "⚠️ چند ترک پیدا نشد",
                            f"{album.downloaded_tracks} از {album.total_tracks} ترک داخل ZIP قرار گرفت.",
                ...
                logger.warning("Spotisaver album failed for %s: %s", request_id, exc)
...
async def handle_private_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not urls:
        await update.effective_message.reply_text(
            status_card("🙃 این که لینک نبود", "یه لینک کامل و معتبر از پلتفرم‌های پشتیبانی‌شده برام بفرست."),
            parse_mode=ParseMode.HTML,
        )
        return
