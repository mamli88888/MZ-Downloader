...
from spotify_search import (
    MAX_RESULTS as SPOTIFY_SEARCH_MAX_RESULTS,
    RESULTS_PER_PAGE as SPOTIFY_RESULTS_PER_PAGE,
    SpotifySearchError,
    SpotifySearchResult,
    SpotifySearchService,
    normalize_search_query as normalize_spotify_search_query,
)
...
class SpotifySearchSession:
    ...
    reply_to: int | None
    query: str
    results: tuple[SpotifySearchResult, ...]
    current_page: int = 0
    busy: bool = False
    selected: bool = False
...
SPOTIFY_SEARCH_SESSIONS: dict[str, SpotifySearchSession] = {}
...
SPOTIFY_SEARCH_RATE_LIMITS: dict[tuple[int, int], deque[float]] = defaultdict(deque)
ACTIVE_YOUTUBE_SEARCHES: set[tuple[int, int]] = set()
ACTIVE_SPOTIFY_SEARCHES: set[tuple[int, int]] = set()
...
def allow_spotify_search(key: tuple[int, int]) -> bool:
    now = time.monotonic()
    history = SPOTIFY_SEARCH_RATE_LIMITS[key]
    while history and now - history[0] > YOUTUBE_SEARCH_RATE_WINDOW:
        history.popleft()
    if len(history) >= YOUTUBE_SEARCH_RATE_LIMIT:
        return False
...
def spotify_search_page_count(session: SpotifySearchSession) -> int:
    return max(1, math.ceil(len(session.results) / SPOTIFY_RESULTS_PER_PAGE))
...
def spotify_search_keyboard(session: SpotifySearchSession, page: int) -> InlineKeyboardMarkup:
    start = page * SPOTIFY_RESULTS_PER_PAGE
    stop = min(start + SPOTIFY_RESULTS_PER_PAGE, len(session.results))
    rows: list[list[InlineKeyboardButton]] = []
    row: list[InlineKeyboardButton] = []
    for result_index in range(start, stop):
        row.append(
    ...
    if page + 1 < spotify_search_page_count(session):
        navigation.append(
...
def spotify_search_caption(session: SpotifySearchSession, page: int) -> str:
    page_count = spotify_search_page_count(session)
    return (
        "🟢 <b>نتایج جست‌وجوی Spotify</b>\n"
        f"عبارت: <b>{html_escape(session.query)}</b>\n"
        f"صفحه {persian_number(page + 1)} از {persian_number(page_count)}"
        " • یکی از محتواها را انتخاب کن."
    )
...
def prune_spotify_search_sessions() -> None:
    now = time.monotonic()
    for token, session in list(SPOTIFY_SEARCH_SESSIONS.items()):
        if now - session.created_at >= YOUTUBE_SEARCH_TTL:
            SPOTIFY_SEARCH_SESSIONS.pop(token, None)
    overflow = len(SPOTIFY_SEARCH_SESSIONS) - MAX_YOUTUBE_SEARCH_SESSIONS
    if overflow > 0:
        oldest = sorted(SPOTIFY_SEARCH_SESSIONS.values(), key=lambda item: item.created_at)[:overflow]
        for session in oldest:
            SPOTIFY_SEARCH_SESSIONS.pop(session.token, None)
    for key, history in list(SPOTIFY_SEARCH_RATE_LIMITS.items()):
        while history and now - history[0] > YOUTUBE_SEARCH_RATE_WINDOW:
            history.popleft()
        if not history:
            SPOTIFY_SEARCH_RATE_LIMITS.pop(key, None)
...
async def run_spotify_search(
    ...
    key = request_owner_key(update)
    try:
        query_text = normalize_spotify_search_query(raw_query)
    except SpotifySearchError:
        await message.reply_text(
            status_card(
                "🟢 چی رو جست‌وجو کنم؟",
                "عبارت موردنظرت رو بنویس؛ مثلاً <code>/ssearch آهنگ جدید</code>.",
            ),
            parse_mode=ParseMode.HTML,
        )
        return
    if key in ACTIVE_SPOTIFY_SEARCHES:
        await message.reply_text(
            status_card("⏳ جست‌وجوی قبلی هنوز ادامه دارد", "چند لحظه صبر کن تا نتیجه‌ها آماده شوند."),
            parse_mode=ParseMode.HTML,
        )
        return
    if not allow_spotify_search(key):
        await message.reply_text(
            status_card("⏱ کمی آهسته‌تر", "تا یک دقیقهٔ دیگر دوباره جست‌وجو کن."),
            parse_mode=ParseMode.HTML,
        )
    ...
    prune_spotify_search_sessions()
    MEMBERSHIP_CACHE[update.effective_user.id] = max(
        MEMBERSHIP_CACHE.get(update.effective_user.id, 0),
        time.monotonic() + YOUTUBE_SEARCH_TTL,
    )
    ACTIVE_SPOTIFY_SEARCHES.add(key)
    status_message: Any | None = None
    session: SpotifySearchSession | None = None
    try:
        status_message = await send_status(
            context,
            update.effective_chat.id,
            status_card(
                "🟢 دارم توی Spotify می‌گردم…",
                f"عبارت: <b>{html_escape(query_text)}</b>\n۳۰ نتیجهٔ مرتبط رو مرتب می‌کنم.",
            ),
            message.message_id,
        )
        results = await SPOTIFY_SEARCH.search(query_text)
        token = uuid.uuid4().hex[:12]
        session = SpotifySearchSession(
            token=token,
            created_at=time.monotonic(),
            chat_id=update.effective_chat.id,
            user_id=update.effective_user.id,
            reply_to=message.message_id,
            query=query_text,
            results=results[:SPOTIFY_SEARCH_MAX_RESULTS],
        )
        page_image = await SPOTIFY_SEARCH.build_page_image(session.results, 0)
        SPOTIFY_SEARCH_SESSIONS[token] = session
        prune_spotify_search_sessions()
        ...
        async def send_results() -> Any:
            return await context.bot.send_photo(
                chat_id=session.chat_id,
                photo=page_image,
                filename=f"spotify-search-{token}-1.jpg",
                caption=spotify_search_caption(session, 0),
                parse_mode=ParseMode.HTML,
                reply_markup=spotify_search_keyboard(session, 0),
                reply_to_message_id=session.reply_to,
            )
        ...
        logger.info("Spotify search unavailable: %s", exc)
        if status_message is not None:
            await edit_status(
                status_message,
                status_card(
                    "😕 نتیجه‌ای آماده نشد",
                    "جست‌وجوی Spotify موقتاً پاسخ نداد یا نتیجه‌ای پیدا نشد.",
                    "عبارت دیگری را امتحان کن.",
                ),
            )
    except TelegramError as exc:
        logger.warning("Spotify search result could not be sent: %s", exc)
        if session is not None:
            SPOTIFY_SEARCH_SESSIONS.pop(session.token, None)
        if status_message is not None:
            await edit_status(
        ...
        logger.exception("Unexpected Spotify search failure: %s", exc)
        if session is not None:
            SPOTIFY_SEARCH_SESSIONS.pop(session.token, None)
        if status_message is not None:
            await edit_status(
        ...
        ACTIVE_SPOTIFY_SEARCHES.discard(key)
...
@membership_required
async def spotify_search_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query_text = " ".join(context.args or ())
    if not query_text and update.effective_message.reply_to_message is not None:
        replied = update.effective_message.reply_to_message
        query_text = replied.text or replied.caption or ""
    await run_spotify_search(update, context, query_text)
...
@membership_required
async def on_spotify_search_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query = update.callback_query
    data = query.data or ""
    parts = data.split(":")
    token = parts[1] if len(parts) > 1 else ""
    prune_spotify_search_sessions()
    session = SPOTIFY_SEARCH_SESSIONS.get(token)
    if session is None:
        await query.answer("این جست‌وجو منقضی شده؛ دوباره جست‌وجو کن.", show_alert=True)
        return
    if session.chat_id != update.effective_chat.id or session.user_id != update.effective_user.id:
    ...
    if data.startswith("ss:"):
        ...
        selected = session.results[value]
        selected_url = normalize_url(selected.url)
        if selected_url is None or detect_platform(selected_url) != Platform.SPOTIFY:
            session.selected = False
            await query.answer("لینک این محتوا معتبر نیست.", show_alert=True)
            return
        with contextlib.suppress(TelegramError):
    ...
    try:
        with contextlib.suppress(TelegramError):
            await query.answer("دارم صفحه را می‌سازم…")
        page_image = await SPOTIFY_SEARCH.build_page_image(session.results, value)
        if SPOTIFY_SEARCH_SESSIONS.get(token) is not session or session.selected:
            return
        await query.edit_message_media(
            media=InputMediaPhoto(
                media=page_image,
                filename=f"spotify-search-{token}-{value + 1}.jpg",
                caption=spotify_search_caption(session, value),
                parse_mode=ParseMode.HTML,
            ),
            reply_markup=spotify_search_keyboard(session, value),
        )
        session.current_page = value
    except (SpotifySearchError, TelegramError) as exc:
        logger.warning("Spotify search page %d failed: %s", value, exc)
        with contextlib.suppress(TelegramError):
            await context.bot.send_message(
                chat_id=session.chat_id,
                text=status_card("😕 صفحه آماده نشد", "دوباره روی دکمهٔ صفحه بزن."),
...
WELCOME_PRIVATE = status_card(
    ...
    "🟢 جست‌وجوی Spotify و نمایش ۳۰ نتیجه\n"
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    text = status_card(
        ...
        "<code>/ssearch عبارت</code> جست‌وجوی Spotify\n"
...
async def post_init(application: Application) -> None:
    ...
    private_commands = [
        ...
        BotCommand("ssearch", "جست‌وجو در Spotify"),
...
async def post_shutdown(application: Application) -> None:
    ...
    SPOTIFY_SEARCH_SESSIONS.clear()
    ACTIVE_YOUTUBE_SEARCHES.clear()
    ACTIVE_SPOTIFY_SEARCHES.clear()
...
def main() -> None:
    ...
    application.add_handler(CommandHandler(("ssearch", "sp"), spotify_search_command))
    ...
    application.add_handler(CallbackQueryHandler(on_spotify_search_callback, pattern=r"^(?:ss|sp):"))
