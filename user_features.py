...
# Comprehensive, always-current bot context handed to the AI so /ask answers
# are grounded in what the bot ACTUALLY does (previously the model had zero
# context and invented answers). Kept in sync with /help — every command,
# platform capability and behaviour a user can ask about.
BOT_HELP_CONTEXT = """«MZ-Downloader» یک ربات دانلودر تلگرام است: لینک محتوا را بفرست تا تشخیص داده شود، منوی گزینه‌ها (کیفیت/نوع فایل) با حجم تقریبی هر گزینه نمایش داده شود و بعد از انتخاب، فایل ارسال شود.

■ نحوه استفاده
· چت خصوصی: لینک را مستقیم بفرست (تا چند لینک در یک پیام). برای جست‌وجوی یوتیوب فقط عبارت را بنویس.
· گروه: «/dl لینک» یا ریپلای روی پیام لینک‌دار و ارسال /dl یا منشن ربات کنار لینک. ربات باید عضو گروه باشد (از پروفایل ربات → Add to Group → سپس ادمین کن).
· برای استفاده از ربات، عضویت در کانال(های) مشخص‌شده الزامی است؛ اگر عضو نباشی دکمه تأیید عضویت نشان داده می‌شود.

■ پلتفرم‌ها و قابلیت‌ها
· اینستاگرام: پست تکی، آلبوم/کروسل چند اسلایدی (همه عکس‌ها به‌صورت آلبوم کامل و مرتب ارسال می‌شود)، ریلز، IGTV و TV. منو شامل «ویدیو (کیفیت اصلی)»، «تصاویر (همه اسلایدها)» و «فقط صدا MP3» است. کپشن پست هم قابل دریافت است. زیر ریلز پیشنهاد موزیک همان ریلز هم می‌آید.
· اینستاگرام پروفایل: «/profile نام‌کاربری» مثل /profile natgeo — کارت پروفایل (عکس پروفایل با کیفیت اصلی، فالوور/فالووینگ، تعداد پست) + دکمه «دانلود آخرین پست» که آخرین پست را با موتور خود ربات دانلود می‌کند.
· یوتیوب: ویدیو و Shorts با کیفیت‌های 144p تا 4K، گزینه «فقط صدا (MP3)»، زیرنویس با دکمه «🈶 زیرنویس» و انتخاب زبان (فارسی/انگلیسی/...) که فایل زیرنویس ارسال می‌شود.
· جست‌وجوی یوتیوب: «/search عبارت» یا نوشتن مستقیم عبارت در چت خصوصی.
· تیک‌تاک: ویدیو بدون واترمارک با بهترین کیفیت.
· توییتر/X: ویدیو با کیفیت اصلی یا نسخه کم‌حجم، همه تصاویر توییت به‌صورت آلبوم، و متن کامل توییت با آمار (لایک/ریتوییت/ریپلای).
· فیسبوک: ویدیو/ریل عمومی + گزینه «فقط صدا (MP3)».
· اسپاتیفای: لینک تراک → MP3 با متادیتای کامل (نام تراک، خواننده، آلبوم، کاور)؛ آلبوم و پلی‌لیست به‌صورت فایل ZIP.
· ساوندکلاد: تراک با بهترین کیفیت MP3؛ پلی‌لیست‌ها هم پشتیبانی می‌شوند.
· پینترست: پین تصویری با کیفیت اصلی، پین ویدیویی هم دانلود می‌شود؛ ربات خودش نوع پین را تشخیص می‌دهد و فقط گزینه‌های مرتبط نشان می‌دهد.
· VK و اکثر لینک‌های عمومی ویدیو/موزیک دیگر هم پشتیبانی می‌شوند.

■ دستورها
/start شروع و راهنما · /help راهنمای کامل · /platforms و /services فهرست پلتفرم‌ها · /status وضعیت صف دانلود · /cancel توقف دانلودهای در حال انجام خودت
/search عبارت — جست‌وجوی یوتیوب
/song عبارت — جست‌وجوی آهنگ؛ نتیجه با کاور و نام خواننده/آهنگ ارسال می‌شود (مثل Shazam)
/caption لینک — فقط متن کپشن پست اینستاگرام را می‌فرستد
/profile نام‌کاربری — کارت پروفایل اینستاگرام + دانلود آخرین پست
/bookmarks — فهرست محتواهای ذخیره‌شده‌ات؛ زیر هر فایل دانلودشده دکمه «🔖 ذخیره» می‌آید
/mystats — آمار شخصی ۳۰ روز اخیر: تعداد دانلود، حجم کل، پلتفرم‌های پراستفاده و نمودار روزانه
/stats — آمار همین اجرای ربات
/autoshare add — داخل کانال/گروه خودت (ربات باید ادمین باشد) مقصد ثبت کن؛ از آن بعد هر دانلودت خودکار آنجا هم ارسال می‌شود. حذف با /autoshare del
/schedule لینک بازه — دانلود خودکار دوره‌ای؛ بازه‌ها: 90m و 12h و 1d و 7d و 2w (حداقل 60 دقیقه؛ حداکثر ۲۰ زمان‌بندی برای هر کاربر). مثال: /schedule https://... 7d
/ask سوال — همین دستیار هوشمند؛ جواب سوالات درباره ربات

■ هوش مصنوعی
· زیر کپشن اینستاگرام و متن توییت دکمه «🤖 خلاصه کن» می‌آید: خلاصه فارسی همان محتوا.
· دستیار /ask با Gemini رایگان (سرویس prexzyapis) جواب می‌دهد؛ کلید API لازم ندارد.

■ رفتارهای فنی
· محتوای تکراری بدون دانلود مجدد و آنی از کش سرور ارسال می‌شود.
· فایل‌های بزرگ‌تر از حد مجاز تلگرام به‌صورت خودکار روی فضای ابری آپلود و لینک دانلود موقت ارسال می‌شود؛ پس عملاً محدودیت حجمی حس نمی‌شود (محدودیت‌های فنی تلگرام و سرور سر جایش است).
· حجم تقریبی هر گزینه کیفیت، کنار همان دکمه نوشته شده تا قبل از دانلود بدانی.
· دانلودها صف می‌شوند؛ با /status وضعیت را ببین و با /cancel لغو کن.

■ راهنمای پاسخ‌دهی تو (دستیار)
فقط فارسی و کوتاه و دقیق جواب بده؛ بر اساس همین راهنما. اگر جواب در راهنما نبود صادقانه بگو و به /help ارجاع بده. لینک خصوصی/پولی/حذف‌شده دانلود نمی‌شود؛ برای محتوای خصوصی راهی وجود ندارد."""
