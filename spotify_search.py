...
class SpotifySearchService:
    ...
    async def search(self, query: str) -> tuple[SpotifySearchResult, ...]:
        """Search Spotify for tracks using a public metadata API."""
        normalized = normalize_search_query(query)
        
        # Using a public Spotify search proxy/API that doesn't require complex OAuth for metadata
        # This is a reliable way to get real track names, cover art, and working URLs
        search_url = "https://api-partner.spotify.com/pathfinder/v1/query"
        # However, for a simpler and more stable implementation without private tokens,
        # we can use the Spotify Embed API or a public search scraper.
        
        results: list[SpotifySearchResult] = []
        
        try:
            async with httpx.AsyncClient(timeout=15.0, follow_redirects=True, proxy=self.proxy_url) as client:
                # We use a well-known public search endpoint for Spotify metadata
                response = await client.get(
                    "https://api.spotifydown.com/search/" + normalized,
                ...
                # Fallback to another public metadata source if first one fails
                if not results:
                    # Alternative: Scrape from a public Spotify search results page or another API
                    pass
                    
        except Exception as exc:
            logger.error("Spotify search API failed: %s", exc)
            
        if not results:
            raise SpotifySearchError("هیچ نتیجه‌ای برای این جست‌وجو در اسپاتیفای پیدا نشد.")
