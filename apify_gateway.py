...
class ApifyGateway:
    ...
    async def request(
        ...
        *,
        ...
        """Return a button menu; do not spend an Apify credit before selection."""
        del attempt_directory, progress_callback  # Kept for the common gateway contract.
        if not self.tokens:
    ...
    async def select(
        ...
        *,
        ...
        try:
            ...
            result = await self._run_with_failover(actor_id, actor_input)
    ...
    async def _run_with_failover(self, actor_id: str, actor_input: dict[str, Any]) -> dict[str, Any]:
        ...
        for token_index in await self._token_candidates():
            try:
                async with httpx.AsyncClient(
                    ...
                    run = await self._run_actor(client, actor_id, actor_input)
