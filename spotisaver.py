...
COLLECTION_PATTERN = re.compile(r"^/(album|playlist)/([A-Za-z0-9]{10,32})/?$")
...
def spotify_collection(url: str) -> tuple[str, str]:
    try:
        parsed = urlsplit(url)
    except ValueError as exc:
        raise SpotisaverError("Invalid Spotify collection URL") from exc
    host = (parsed.hostname or "").lower().strip(".")
    if parsed.scheme != "https" or host not in {"open.spotify.com", "spotify.com", "www.spotify.com"}:
        raise SpotisaverError("Only public open.spotify.com album or playlist links are supported")
    match = COLLECTION_PATTERN.fullmatch(parsed.path)
    if not match:
        raise SpotisaverError("Spotify link is not an album or playlist")
    return match.group(1), match.group(2)


def spotify_album_id(url: str) -> str:
    resource_type, resource_id = spotify_collection(url)
...
def spotify_playlist_id(url: str) -> str:
    ...
    if resource_type != "playlist":
        raise SpotisaverError("Spotify link is not a playlist")
    return resource_id
...
class SpotisaverAlbumDownloader:
    def __init__(
        ...
        *,
        ...
        self.page_slug = "album-downloader"
        self.headers = {
            "User-Agent": (
    ...
    def _ajax_headers(self) -> dict[str, str]:
        return {
            **self.headers,
            ...
            "Referer": f"{BASE_URL}/en/{self.page_slug}",
    ...
    async def download_collection(
        ...
        *,
        ...
        resource_type, resource_id = spotify_collection(url)
        self.page_slug = f"{resource_type}-downloader"
        collection_label = "آلبوم" if resource_type == "album" else "پلی‌لیست"
        fallback_name = "Spotify Album" if resource_type == "album" else "Spotify Playlist"
        ...
        async with httpx.AsyncClient(
            ...
            await _notify(progress, 2, f"گرفتن اطلاعات {collection_label}")
            page = await client.get(f"{BASE_URL}/en/{self.page_slug}")
            page.raise_for_status()
            match = re.search(r'const\s+user_ip\s*=\s*["\']([^"\']*)', page.text)
            user_ip = match.group(1) if match else ""
            context = {"id": resource_id, "type": resource_type, "lang": "en"}
            ...
            if not isinstance(tracks, list) or not tracks:
                raise SpotisaverError("Spotisaver returned no collection tracks")
            if len(tracks) > 100:
                raise SpotisaverError("Spotisaver free collection limit is 100 tracks")
            info = data.get("playlist_info") if isinstance(data.get("playlist_info"), dict) else {}
            collection_name = _safe_name(str(info.get("name") or fallback_name), fallback_name)
            ...
            if not downloaded:
                raise SpotisaverError("No collection track could be downloaded")
            await _notify(progress, 90, "ساخت فایل ZIP")
            zip_path = directory / f"{collection_name}.zip"
            await asyncio.to_thread(_zip_and_remove, downloaded, zip_path)
            await _notify(progress, 100, f"{collection_label} آماده است")
            return SpotifyAlbumResult(zip_path, collection_name, len(downloaded), total, tuple(failed))
    ...
    async def download_album(
        ...
        *,
        ...
        spotify_album_id(url)
        return await self.download_collection(url, directory, progress=progress)
