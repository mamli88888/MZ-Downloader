...
git init
git branch -M main
git add .gitignore .dockerignore .env.example bot.py config.py downloader.py instagram_caption.py spotisaver.py youtube_search.py routing.py requirements.txt Dockerfile railway.json Procfile README.md RAILWAY_DEPLOY_FA.md export_sessions.py tests
git status --short
```
...
# لایه سرعت (پیش‌فرض‌ها مناسب است)
...
- **لایه سرعت (مهم):** اندازه‌گیری زنده نشان داد سرور voiddl هر درخواست را از نو آماده می‌کند (۱۵ تا ۷۶ ثانیه بسته به حجم و بار سرور، بدون کش) اما آماده‌سازی درخواست‌های هم‌زمان به‌صورت موازی انجام می‌شود؛ همچنین پهنای باند دانلود به ازای هر کلید حدود ۱۲–۱۵ مگابایت بر ثانیه سقف دارد. برای همین ربات: (۱) فایل‌های بزرگ را با چند درخواست Range هم‌زمان (پیش‌فرض ۸ لاین) دانلود می‌کند و (۲) همین که کارت کیفیت نمایش داده شود، محتمل‌ترین کیفیت‌ها (1080p، 720p و MP3) را در پس‌زمینه شروع می‌کند تا زمان آماده‌سازی سرور با زمان انتخاب کاربر هم‌پوشانی شود. **هرچه کلید بیشتری در `VOIDDL_API_KEYS` بگذارید، سرعت انتقال چند برابر می‌شود** (هر کلید یک لوله‌ی مستقل ~۱۲–۱۵MB/s و ۱۰ گیگ در روز است)؛ برای فایل‌های ۲۰۰–۳۰۰ مگی ۳–۴ کلید توصیه می‌شود. با `VOIDDL_PREFETCH=false` می‌توانید پری‌فچ را خاموش کنید و `VOIDDL_PARALLEL_LANES` تعداد لاین‌های موازی دانلودِ پس از انتخاب کیفیت است.
...
5. فرمان `/search عبارت` را بفرستید، پنج صفحهٔ نتایج را ورق بزنید و انتخاب یکی از محتواها را تا منوی دانلود بررسی کنید.
6. Twitter/X، Facebook و VK را بررسی کنید؛ فقط واسط اول باید استفاده شود.
7. یک Spotify track، یک Spotify album و یک SoundCloud را بررسی کنید؛ آلبوم باید ZIP شود.
8. یک فایل بزرگ‌تر از 30 MB را آزمایش کنید؛ باید در چند بخش ارسال شود.
9. `/cancel`, `/status`, `/platforms`, `/stats` و `/help` را بررسی کنید.
10. Service را Restart و دوباره `/health` را بررسی کنید.

## 14. رفع خطاهای رایج

### Build موفق است ولی `instagram_caption` پیدا نمی‌شود

Root Directory اشتباه است یا Dockerfile قدیمی deploy شده. آخرین commit را بررسی و Redeploy کنید. Dockerfile فعلی `instagram_caption.py`، `spotisaver.py` و `youtube_search.py` را کپی می‌کند.
